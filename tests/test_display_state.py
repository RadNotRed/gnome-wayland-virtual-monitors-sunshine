...
from gnome_virtual_monitors.display_state import (
    apply_layout_payload, compose_layout, physical_anchor, resolve_virtual_roles,
    snapshot_layout, validate_preserved_modes,
)
from gnome_virtual_monitors.model import DaemonConfig, PrimaryConfig, VirtualMonitorConfig


def monitor(connector, width=1920, height=1080, refresh=143.9814453125, virtual=False):
    spec = (connector, "fake-vendor", "Virtual remote monitor" if virtual else "fake-model", "fake")
    mode = (f"{connector}-exact-mode", width, height, refresh, 1.0, [1.0, 1.5, 2.0], {"is-current": True})
    return (spec, [mode], {})
...
def configuration(**kwargs):
    return DaemonConfig(
        ...
        (VirtualMonitorConfig("client", 1280, 720, 60, 1, "primary", "below", "start"),),
        1, 1, 15, **kwargs,
    )
...
class DisplayStateTests(unittest.TestCase):
    def test_one_two_and_three_physical_monitors_are_preserved_exactly(self):
        for count in (1, 2, 3):
            with self.subTest(count=count):
                physical = [monitor(f"physical-{i}", refresh=60.001 + i * 83.98) for i in range(count)]
                groups = [logical(m, x=1920*i, primary=i == 0) for i, m in enumerate(physical)]
                original = snapshot_layout(physical, groups)
                virtual = monitor("Meta-5", 1280, 720, 60, True)
                roles = resolve_virtual_roles(configuration(), physical + [virtual], set())
                result = compose_layout(original, physical_anchor(configuration(), original, physical), roles)
    ...
    def test_portrait_scale_and_current_mode_drive_anchor_geometry(self):
        ...
        groups = snapshot_layout([physical], [logical(physical, scale=1.5, transform=1)])
        virtual = monitor("Meta-7", 1280, 720, 60, True)
        roles = resolve_virtual_roles(configuration(), [virtual], set())
        result = compose_layout(groups, physical_anchor(configuration(), groups, [physical]), roles)
    ...
    def test_mixed_scales_and_transforms(self):
        ...
        layout = snapshot_layout(outputs, [logical(outputs[0]), logical(outputs[1], x=1920, scale=2, transform=3, primary=False)])
    ...
    def test_additional_physical_and_preexisting_virtual_are_not_role_candidates(self):
        outputs = [monitor("physical-A", 1280, 720, 60), monitor("Meta-0", 1280, 720, 60, True), monitor("Meta-9", 1280, 720, 60, True)]
        self.assertEqual(resolve_virtual_roles(configuration(), outputs, {"Meta-0"})[0].connector, "Meta-9")

    def test_recreated_role_uses_new_connector(self):
        for name in ("Meta-0", "Meta-9"):
            role = resolve_virtual_roles(configuration(), [monitor(name, 1280, 720, 60, True)], set())[0]
    ...
    def test_ambiguous_virtual_role_is_rejected(self):
        with self.assertRaisesRegex(RuntimeError, "could not be uniquely matched"):
            resolve_virtual_roles(configuration(), [monitor(name, 1280, 720, 60, True) for name in ("Meta-0", "Meta-1")], set())
    ...
    def test_left_placement_translates_all_groups_by_same_offset_only(self):
        outputs = [monitor("physical-A"), monitor("physical-B")]
        layout = snapshot_layout(outputs, [logical(outputs[0]), logical(outputs[1], x=1920, primary=False)])
        config = configuration()
        config = replace(config, monitors=(replace(config.monitors[0], position="left"),))
        virtuals = resolve_virtual_roles(config, [monitor("Meta-8", 1280, 720, 60, True)], set())
