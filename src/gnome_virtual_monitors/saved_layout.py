...
def validate_saved_setup(
    ...
    available = {m[0][0] for m in monitors if not is_virtual(m)}
    saved_physical = {
        output["identity"]
...
def restore_layout(
    ...
    for group in state["logical_monitors"]:
        ...
        for saved in group["outputs"]:
            ...
            if len(matching) != 1:
                raise RuntimeError(
                    f"Saved mode for {identity!r} is unavailable or ambiguous"
                )
            mode = matching[0]
            if not any(abs(scale - group["scale"]) < 1e-6 for scale in mode[5]):
                raise RuntimeError(f"Saved scale for {identity!r} is unavailable")
            outputs.append(MonitorMode(connector, *mode[:4]))
        result.append(
            LogicalMonitor(
                group["x"],
                group["y"],
