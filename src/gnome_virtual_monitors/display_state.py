...
def snapshot_layout(
    monitors: Sequence, logical_monitors: Sequence
) -> tuple[LogicalMonitor, ...]:
    ...
    for x, y, scale, transform, primary, specs, _props in logical_monitors:
        ...
        for spec in specs:
            ...
            current = (
                []
                if monitor is None
                else [mode for mode in monitor[1] if mode[6].get("is-current", False)]
            )
            if len(current) != 1:
                raise RuntimeError(
                    f"Cannot uniquely resolve current mode for {spec[0]}"
                )
            mode = current[0]
...
def validate_preserved_modes(
    layout: Sequence[LogicalMonitor], monitors: Sequence
) -> None:
    ...
    for logical in layout:
        for output in logical.monitors:
            ...
            if monitor is None:
                raise RuntimeError(
                    f"Preserved connector {output.connector} is no longer available"
                )
            modes = [
                mode
            ...
            if len(modes) != 1 or logical.scale not in modes[0][5]:
                raise RuntimeError(
                    f"Preserved mode/scale for {output.connector} is no longer available"
                )
    ...
    for profile in config.monitors:
        ...
        resolved.append(
            ResolvedMonitor(
                ...
                *mode[:4],
                (
                    choose_scale(mode[5], profile.scale)
...
def physical_anchor(
    ...
    matches = [
        ...
        if output.connector in physical
        and (
            output.connector == config.primary.connector
            if config.primary.connector
            else group.primary
        )
    ]
    if not matches:
...
def compose_layout(
    ...
    placed = place_monitors(
        [reference, *virtuals], origin=(group.x, group.y), normalize=False
    )
    layout = [*preserved, *layout_from_placements(placed[1:])]
...
def layout_from_placements(placed: Sequence[PlacedMonitor]) -> list[LogicalMonitor]:
    """Convert config-driven placements to the same model used by snapshots."""
    return [
        LogicalMonitor(
            x=item.x,
            y=item.y,
            scale=item.monitor.scale,
            transform=0,
            primary=item.monitor.primary,
            monitors=(
                MonitorMode(
                    connector=item.monitor.connector,
                    mode_id=item.monitor.mode_id,
                    width=item.monitor.width,
                    height=item.monitor.height,
                    refresh=item.monitor.refresh,
                ),
            ),
        )
        for item in placed
...
def layout_summary(layout: Sequence[LogicalMonitor]) -> str:
    groups = []
    for group in layout:
        outputs = ", ".join(
            f"{output.connector}/{output.mode_id} "
            f"{output.width}x{output.height}@{output.refresh:.6g}"
            for output in group.monitors
        )
        groups.append(
            f"{outputs} scale={group.scale:.6g} transform={group.transform} "
            f"primary={group.primary} pos={group.x},{group.y}"
        )
    return "; ".join(groups)
